...
def load_config_files():
    global num_anon_users, system_prompt, current_model
    
    # Load anon users setting
    try:
    ...
    try:
        if os.path.exists('anon_settings.json'):
            with open('anon_settings.json', 'r') as f:
                ...
                if 'num_anon_users' in settings:
                    num_anon_users = max(0, min(7, settings['num_anon_users']))
    except Exception as e:
        print(f"Error refreshing anon_users before post: {e}")
...
# API endpoints for post persistence
...
# API endpoints for system prompt
